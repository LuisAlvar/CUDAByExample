...
__global__ void blend_kernel(float* outSrc, const float* inSrc)
{
  ...
  // x equals to 0 along the entire left side of the grid, there is no left, so left is the offset value
  if (x == 0) left++;
  // x equals to DIM - 1 along the entire right side of the grid, there is no right, so right is the offset value.
  if (x == DIM - 1) right--;

  // By substracting the value of DIM you get the previous row from offset
  int top = offset - DIM;
  // By adding the value of DIM you get the next row from offset
  int bottom = offset + DIM;

  // y equals to zero along the entire top row, there is no top value, so, the top is back to the offset value 
  if (y == 0) top += DIM;
  // y equals to DIM - 1 along the entire bot row, there is bot value, so the bot is back to the offset value
